...
func (gen *myGenerator) Start() bool {
	...
	if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_ORIGINAL, lib.STATUS_STARING) {
		if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_STOPPED, lib.STATUS_STARING) {
			return false
		}
	}
...
func (gen *myGenerator) Stop() bool {
	if !atomic.CompareAndSwapUint32(&gen.status, lib.STATUS_STARTED, lib.STATUS_STOPPED) {
		return false
	}
...
func (gen *myGenerator) callOne(rawReq *lib.RawReq) *lib.RawResp {
	...
	if rawReq == nil {
		return &lib.RawResp{ID: 0, Err: errors.New("Invalid raw request.")}
	}
